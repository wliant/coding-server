...
@pytest.mark.asyncio
async def test_run_coding_agent_returns_failure_on_agent_error():
    """Returns (False, error_msg) when agent returns error."""
    from worker import agent_runner
    ...
    with (
        ...
        mock_agent_instance = MagicMock()
        mock_agent_instance.run.return_value = MagicMock(error="Something went wrong")
        MockConfig = MagicMock(return_value=MagicMock())
        MockAgent = MagicMock(return_value=mock_agent_instance)
        mock_resolve.return_value = (MockAgent, MockConfig)
