...
def test_agent_run_creates_output_file(tmp_working_dir, fake_crew_run):
...
def test_agent_run_returns_non_empty_code(tmp_working_dir, fake_crew_run):
    cfg = _make_config(tmp_working_dir, "calculator", "write an add function")
    result = CodingAgent(cfg).run()
...
def test_agent_run_returns_review(tmp_working_dir, fake_crew_run):
    cfg = _make_config(tmp_working_dir, "calculator", "write an add function")
    result = CodingAgent(cfg).run()
...
def test_agent_run_output_file_in_working_directory(tmp_working_dir, fake_crew_run):
    # crew.py picks the newest .py file it finds — it does not name files after project_name
...
def test_agent_run_creates_working_directory_if_missing(tmp_path, fake_crew_run):
